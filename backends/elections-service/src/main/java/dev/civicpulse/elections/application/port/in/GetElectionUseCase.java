...
public interface GetElectionUseCase {
  ...
  /** Ordered by office then rank — every candidate TSE reported for this election's races, not
   * just the ones who also got a platform account (see ElectionResult's javadoc). */
  List<ElectionResult> listResults(UUID electionId);
}
