...
public interface ProjectDirectoryUseCase {
   ...
   * {@code govLevel} (a {@code gov_level_options} code) is null except when published by
   * government-sync-service — the two admin-driven flows leave the projection's existing level
   * untouched. */
  void onRepresentativeLinked(UUID politicianAccountId, UUID partyId, String roleTitle, String state, String govLevel, Instant linkedAt);
